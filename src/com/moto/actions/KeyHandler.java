...
public class KeyHandler implements DeviceKeyHandler {
    ...
    public KeyEvent handleKeyEvent(KeyEvent event) {
        ...
        boolean isFPGestureEnabledOnScreenOff = getSystemSetting(mContext, FP_HOME_KEY_OFF) == 1;
    ...
    private boolean processFPScancode(int scanCode, boolean isScreenOn) {
        ...
        switch (scanCode) {
            case FP_TAP_SCANCODE:
                action = getSystemSetting(mContext, isScreenOn ? FP_KEYS : FP_KEYS_OFF);
                break;
            case FP_DTAP_SCANCODE:
                action = getSystemSetting(mContext, isScreenOn ? FP_KEY_DBLTAP : FP_KEY_DBLTAP_OFF);
                break;
            case FP_HOLD_SCANCODE:
                action = getSystemSetting(mContext, isScreenOn ? FP_KEY_HOLD : FP_KEY_HOLD_OFF);
                break;
            case FP_UP_SCANCODE:
                action = getSystemSetting(mContext, isScreenOn ? FP_KEY_UP : FP_KEY_UP_OFF);
                break;
            case FP_DOWN_SCANCODE:
                action = getSystemSetting(mContext, isScreenOn ? FP_KEY_DOWN : FP_KEY_DOWN_OFF);
                break;
            case FP_LEFT_SCANCODE:
                action = getSystemSetting(mContext, isScreenOn ? FP_KEY_LEFT : FP_KEY_LEFT_OFF);
                break;
        ...
        Log.i(TAG, "isActionSupported " + isActionSupported);
    ...
    private void fireGoogleNowOnTap() {
        ...
        if (mSearchManagerService != null) {
            try {
                mSearchManagerService.launchAssist(UserHandle.USER_CURRENT, new Bundle());
    ...
    private void doHapticFeedbackScreenOff() {
        ...
        boolean enabled = getSystemSetting(mContext, FP_HAPTIC_SCREENOFF_KEY) == 1;
        if (enabled) {
